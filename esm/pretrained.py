...
def load_model_and_alphabet_local(model_location):
    """Load from local path. Regression weights are loaded if present, otherwise skipped."""
    model_location = Path(model_location)
    model_data = torch.load(str(model_location), map_location="cpu")
    model_name = model_location.stem
    regression_data = None
    if _has_regression_weights(model_name):
        regression_location = Path(str(model_location.with_suffix("")) + "-contact-regression.pt")
        if regression_location.exists():
            regression_data = torch.load(str(regression_location), map_location="cpu")
        else:
            import warnings
            warnings.warn(
                f"Contact-regression weights not found at {regression_location}. "
                "Skipping regression data (contact prediction will be unavailable, "
                "but sequence optimization is unaffected).",
                UserWarning,
                stacklevel=2,
            )
    return load_model_and_alphabet_core(model_name, model_data, regression_data)
